...
module.exports.listTurnDownloadFiles = function(redis, func) {
    async.waterfall([
        ...
        function(arrayResult, callback) {
            getShortSourcesInformationTurn(redis, arrayResult, function(err, result) {
                if (err) callback(err);
...
module.exports.listImplementationDownloadFiles = function(redis, func) {
    ...
    async.waterfall([
        ...
        function(arrayResult, callback) {
            getShortSourcesInformationImplementation(redis, arrayResult, function(err, result) {
                if (err) callback(err);
                else callback(null, result);
            });
        },
        ...
        function(objResult, callback) {
            for (let key in objResult) {
                for (let sourceIp in objGlobal.downloadFilesTmp) {
                    if (key === objGlobal.downloadFilesTmp[sourceIp].taskIndex) {
                        if (typeof objGlobal.downloadFilesTmp[sourceIp].fileUploadedPercent === 'undefined') continue;

                        let taskIndex = objGlobal.downloadFilesTmp[sourceIp].taskIndex;
                        objResult[taskIndex].fileUploadedPercent = objGlobal.downloadFilesTmp[sourceIp].fileUploadedPercent;
                    }
                }
            }
...
function getShortSourcesInformationImplementation(redis, arrayResult, done) {
    let obj = {};
    async.map(arrayResult, function(item, callbackMap) {
        let arrTmp = item.split(':');
        let sourceId = arrTmp[0];
            ...
            function(err, arrayData) {
                ...
                redis.hget('remote_host:settings:' + sourceId, 'shortName', function(error, shortName) {
                    ...
                    obj[item] = {
                        'sourceId': arrayData[0],
                        'countFilesFound': arrayData[1],
                        'countFilesLoaded': arrayData[2],
                        'countFilesLoadedError': arrayData[3],
                        'shortName': shortName
                    };
                    callbackMap(null, obj);
                });
